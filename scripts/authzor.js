...
$(document).ready(function () {
    ...
    $('#ok-button').click(function (event) {
        ...
        $.ajax({
            ...
            async: false, // Block until validate_path returns so that the user can't perform
            // other actions until submission is complete
            data: {
        ...
        success: function (new_settings) {
            if (new_settings['error']) {
                ...
                if (settings !== null) {
                    ...
                    $('#groups-name-attr-input').val(settings[
                        'groups_name_attr']);
                    $('#group-member-attr-input').val(settings[
                        'group_member_attr']);
                }
            }
        }
...
function commitChanges() {
    // Apply pending changes to the "rules" array (prior to applying it to the real authz)
...
function comparePathThenName(rule_a, rule_b) {
    var path_difference = compareStrings(getFullPath(rule_a), getFullPath(
        rule_b));
...
function showRules() {
    ...
    var sections = [];
    var arrange_by_path = $('#arrange-by-path-radio-button').is(':checked');
...
function getDisplayName(rule) {
    ...
    if (rule["name_type"] == USER) {
        name_type_image =
            '<img src="images/user.png" class="name-type-icon" alt="User" title="User" />';
    } else if (rule["name_type"] == GROUP) {
        name_type_image =
            '<img src="images/group.png" class="name-type-icon" alt="Group" title="Group" />';
    } else {
        name_type_image =
            '<img src="images/asterisk.png" class="name-type-icon" alt="All" title="All" />';
    }
...
function displayError(error) {
    alert(error +
        " Please contact GECO HelpDesk (helpdesk@gecoinc.com) for assistance.");
}
